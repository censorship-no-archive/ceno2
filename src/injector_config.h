...
public:
    ...
    boost::optional<boost::asio::ip::tcp::endpoint> tcp_endpoint() const
...
private:
    ...
    boost::optional<boost::asio::ip::tcp::endpoint> _tcp_endpoint;
...
InjectorConfig::InjectorConfig(int argc, const char**argv)
{
    ...
    if (!exists(_repo_root) || !is_directory(_repo_root)) {
        throw std::runtime_error(ouinet::util::str(
    ...
    if (vm.count("listen-on-tcp")) {
        _tcp_endpoint = ouinet::util::parse_tcp_endpoint(vm["listen-on-tcp"].as<string>());
    }
}
